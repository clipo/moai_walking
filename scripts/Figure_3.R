...
# Using simplified Euclidean approximation for close distances
calculate_distance <- function(lat1, lon1, lat2, lon2) {
  ...
  # Simple Euclidean distance approximation for Easter Island's small area
  # At -27 latitude, 1 degree longitude ≈ 99 km, 1 degree latitude ≈ 111 km
...
road_data_final <- matched_moai %>%
  ...
  mutate(
    ...
    # Calculate CoM using volumetric sectional analysis
    com_position = mapply(calculate_sectional_com_varied, 
                          total_length_cm, 
                          base_width_cm, 
                          shoulder_width)
    # Note: Actual CoM positions may vary by ±5-10% due to geometric assumptions
  ) %>%
...
cat("three-dimensional scanning or additional morphometric measurements, including depth ")
cat("profiles and intermediate width measurements along the moai body.\n\n")
